...
This module provides a practical "ICTC-like" decomposition path that is fast for
...
For L=2 (generic rank-2), we also provide the classical full ICTC decomposition:
...
def decompose_rank2_generic(T: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """
    Generic rank-2 ICTC decomposition (matches the textbook formula):
