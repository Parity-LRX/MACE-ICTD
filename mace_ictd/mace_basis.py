"""Orthogonal ICTD <-> original-MACE (e3nn) basis change.

The baseline ICTD-MACE stores a degree-``l`` feature as the ``2l+1`` components of an *irreducible
...
so an arbitrary equivariant feature ``x`` (ICTD basis) becomes its original-MACE counterpart by a
...
def _q_per_l_f64(lmax: int) -> tuple[torch.Tensor, ...]:
    """Per-degree orthogonal Q_l (float64), deterministic. ICTD harmonics @ Q_l == e3nn SH."""
...
def orthogonal_Q(lmax: int, *, dtype=torch.float64, device="cpu") -> torch.Tensor:
    """Block-diagonal orthogonal ``Q`` (size ``sum_l (2l+1) = (lmax+1)**2``), ICTD -> MACE/e3nn."""
...
def to_mace_basis(x: torch.Tensor, lmax: int) -> torch.Tensor:
    """ICTD -> original-MACE/e3nn. ``x`` has the ``(lmax+1)**2`` angular components in its last axis."""
    Q = orthogonal_Q(int(lmax), dtype=x.dtype, device=x.device)
    return x @ Q


def to_ictd_basis(x: torch.Tensor, lmax: int) -> torch.Tensor:
    """original-MACE/e3nn -> ICTD (inverse of :func:`to_mace_basis`; ``Q`` is orthogonal so Qᵀ)."""
    Q = orthogonal_Q(int(lmax), dtype=x.dtype, device=x.device)
    return x @ Q.transpose(-1, -2)


def to_mace_basis_blocks(blocks: dict[int, torch.Tensor]) -> dict[int, torch.Tensor]:
    """Per-degree dict ``{l: [..., 2l+1]}`` (ICTD) -> original-MACE/e3nn convention."""
    lmax = max(blocks)
